...
Reviewed 2026-08-27. Machine-verifiable pins and checked-in schemas are recorded under
...
description as a documentation error. This resolves `SPEC-01`. The review also found
production order-entry schema v15 and current REST-source deltas that must be adopted under
`SPEC-02` before `ORD-07` begins in the separate restart session.
...
This resolves the external identity gate. The OpenAPI, v15 XML, corrected MD XML, and SDK
version mismatch remain unadopted source work for `SPEC-02`, not reasons to keep `SPEC-01`
open.

## Implemented schema pins

The following are the checked-in references and hardcoded-codec pins. They remain the
implementation baseline until the separate restart session completes `SPEC-02` test-first.
...
- The implemented OE XML v11 has no reduce-only field despite amend prose. XML governs wire layout: reject
...
Before any codec edit, re-download all official sources and compare versions/hashes. The
identity gate is resolved, but the separate restart session must complete `SPEC-02` before
activating `ORD-07`; readiness remains closed until exact reconciliation passes.
