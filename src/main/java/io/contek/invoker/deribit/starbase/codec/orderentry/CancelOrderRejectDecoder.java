...
/** Hardcoded decoder for order-entry schema-v11 CancelOrderReject (template 222). */
public final class CancelOrderRejectDecoder {

  public static final int TEMPLATE_ID = 222;
  ...
  public static void validate(ByteBuffer buffer, int offset) {
    ...
    if (reason < 0 || reason > 8) {
      throw new StarbaseProtocolException("invalid CancelOrderReject reason");
    }
  }
  ...
  public static boolean isOrderIdNull(ByteBuffer buffer, int offset) {
    return orderId(buffer, offset) == Long.MIN_VALUE;
  }
  ...
  private CancelOrderRejectDecoder() {}
}
