...
/** Hardcoded decoder for order-entry schema-v11 AmendOrderResponse (template 210). */
public final class AmendOrderResponseDecoder {
  ...
  public static void validate(ByteBuffer buffer, int offset) {
    ...
    if (fillBlock != FILL_BLOCK_LENGTH || fillCount > 2000) {
      throw new StarbaseProtocolException("invalid AmendOrderResponse fills dimensions");
    }
    long legsLong = (long) fills + GROUP_DIMENSIONS_LENGTH + (long) fillCount * fillBlock;
    if (legsLong > Integer.MAX_VALUE) {
      throw new StarbaseProtocolException("AmendOrderResponse fills range overflow");
    }
    ...
    if (legBlock != LEG_BLOCK_LENGTH) {
      throw new StarbaseProtocolException("invalid AmendOrderResponse legs dimensions");
    }
    long expectedEnd = (long) legs + GROUP_DIMENSIONS_LENGTH + (long) legCount * legBlock;
    if (expectedEnd - offset != messageLength) {
      throw new StarbaseProtocolException("AmendOrderResponse group length mismatch");
    }
    ...
    for (int index = 0; index < legCount; index++) {
      ...
      if (side != 1 && side != -1) {
        throw new StarbaseProtocolException("invalid AmendOrderResponse leg side");
      }
    }
  }
  ...
  private static void validateFixed(ByteBuffer buffer, int offset) {
    ...
    requireRequiredPrice(buffer, body(offset) + 48, "price");
    ...
    if (status < 1 || status > 4) {
      throw new StarbaseProtocolException("invalid AmendOrderResponse status");
    }
    if (reason < 0 || reason > 17) {
      throw new StarbaseProtocolException("invalid AmendOrderResponse cancel reason");
    }
  }

  private static void requireRequiredPrice(ByteBuffer buffer, int offset, String field) {
    if (Price9Codec.isNull(buffer, offset)) {
      throw new StarbaseProtocolException("null AmendOrderResponse " + field);
    }
  }
  ...
  private static void requireRequiredDecimal(
    ...
    if (mantissa == Decimal72Codec.NULL_MANTISSA
        ...
        || positive && mantissa <= 0
        || !positive && mantissa < 0) {
      throw new StarbaseProtocolException("invalid AmendOrderResponse " + field);
    }
  }
  ...
  private static void requireTemplate(ByteBuffer buffer, int offset) {
    ...
    if (actual != TEMPLATE_ID) {
      throw new StarbaseProtocolException(
          "expected AmendOrderResponse template " + TEMPLATE_ID + " but received " + actual);
    }
  }
  ...
  private AmendOrderResponseDecoder() {}
}
