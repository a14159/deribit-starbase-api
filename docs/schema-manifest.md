...
Reviewed 2026-08-27, revalidated on 2026-08-28 and 2026-09-03, and audited again on
2026-09-16. The legacy bundle at
...
schema 2101/version 16/semantic version 1.5, SHA-256
`64EBC71CCAC3A01203977718CD524C9476D311ADFF01049312CA0778E57CF559`. Its v12-v16 delta
...
`MassQuoteRejectReason.MMP_MIN_FREEZE_TIME_NOT_ELAPSED` value 9, and v16 appends required
`quantity` and `totalFilled` `Decimal72` fields to `CancelOrderResponse` (220). The
`OrderId` and `ClientOrderId` types remain `int64`. As of 2026-09-16, the direct testnet
order-entry XML is byte-identical to the production-v16 XML and hash. The direct
market-data XML remains schema 2102/version 1/semantic version 1.0, SHA-256
...
the negotiated session version; it is not necessarily the negotiated v16 ceiling. The
hardcoded layouts remain pinned correctly, and completed `SPEC-03` now applies
schema-derived per-message lower bounds plus the pinned-v16 upper bound before exact
...
| --- | ---: | ---: | ---: | --- | --- |
| Order entry | 2101 | 16 | 1.5 | `src/main/resources/schema/deribit-sbe-order-api.xml` | `64EBC71CCAC3A01203977718CD524C9476D311ADFF01049312CA0778E57CF559` |
...
(68-byte body), `LogonConfirmation` (6-byte body), and the versioned
`CancelOrderResponse`. Encoders stamp production v16, and the assembled lifecycle requires
`LogonConf.schemaVersion` to echo negotiated v16. The dispatcher accepts each template's
schema-derived compatible version floor through the
pinned v16 ceiling, while retaining exact template lengths and bounds checks. In
particular, current `LogonConf`, `Heartbeat`, and `OrderPlaced` layouts accept their
documented header stamps 12, 0, and 8. `CancelOrderResponse` accepts the legacy 56-byte
body through v15 and requires the v16 74-byte body with non-null `quantity` and
`totalFilled`; the public lifecycle validates those values against exact local quantity
and fill state before cancellation. Testnet uses the same v16 XML as production.
